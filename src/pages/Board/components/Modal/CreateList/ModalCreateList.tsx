...
const ModalCreateList = ({ onClose, board_id }: ICreateList) => {
	...
	useEffect(() => {
		if (nameList.length > maxLengthName) setNameError('слишком длинное название');
	...
	return (
		<>
			...
			<button className='add_list_btn_modal' onClick={addNewList} type='submit' disabled={!validForma}>
				Добавить список
			</button>
		</>
	);
